...
__device__ float2 RayAABBIntersection(
  ...
  for (int d = 0; d < 3; ++d) {  
    ...
    f_dim_low  = (aabb - half_voxel - start) * inv_ray_dir; // Distance to wo planes
    ...
    f_low = (f_dim_low > f_low) ? f_dim_low : f_low; // check the new low
    f_high = (f_dim_high < f_high) ? f_dim_high : f_high; // check the new high
    
    // if f_low > f_high, the ray missed
    if (f_low > f_high) {
      return make_float2(-1.0f, -1.0f);
    }
  }
